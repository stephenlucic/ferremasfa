...
async def update_ordenes(id:int, orden:schemas.ordenes, db: Session=Depends(get_db)):
    data=servicio.get_orden(id,db)
    if not data:
        return JSONResponse(status_code=404, content={'message':'no se encontró orden'})
    servicio.update_orden(id,orden,db)
    data=servicio.get_orden(id,db)
    return JSONResponse(status_code=200, content={'message':'se modificó la orden','orden':jsonable_encoder(data)})
